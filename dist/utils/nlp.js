...
exports.extractSmsParamsFromUtterance = extractSmsParamsFromUtterance;
exports.extractMultiplePhones = extractMultiplePhones;
 ...
 */
function extractSmsParamsFromUtterance(utterance) {
    // 안전하게 문자열로 처리
    ...
    const allowNightSend = /(야간\s*(허용|가능|괜찮|ok|가능해|돼))/i.test(text) &&
 ...
 */
function extractMultiplePhones(utterance) {
    const text = (utterance || '').trim();
